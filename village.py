...
async def draw_village_scene(villager_count, fortune_teller_count, werewolf_count):
    text_drawer = TextDrawer(JAPANESE_FONT_FILE)
    text_drawer.draw_text(
        text="役職の合計を(参加人数+2)人にしてください。",
        font_size=FONT_SIZE_S,
        x=ROLE_X,
        y=DESCRIPTION_Y,
    )
    text_drawer.draw_text(text="村人", font_size=FONT_SIZE_M, x=ROLE_X, y=VILLAGER_Y)
    ...
    text_drawer.draw_text(
        text=str(villager_count), font_size=FONT_SIZE_M, x=ROLE_NUMBER_X, y=VILLAGER_Y
    )
    INCREASE_VILLAGER_BUTTON.draw("+")
    text_drawer.draw_text(
        text="占い師", font_size=FONT_SIZE_M, x=ROLE_X, y=FORTUNE_TELLER_Y
    )
    ...
    text_drawer.draw_text(text="人狼", font_size=FONT_SIZE_M, x=ROLE_X, y=WEREWOLF_Y)
    ...
    text_drawer.draw_text(
        text=str(werewolf_count), font_size=FONT_SIZE_M, x=ROLE_NUMBER_X, y=WEREWOLF_Y
    )
    INCREASE_WEREWOLF_BUTTON.draw("+")
    START_BUTTON.draw("スタート")


async def update_village_scene(
    villager_count, fortune_teller_count, werewolf_count, players: list
):
    scene = SCENE_VILLAGE
